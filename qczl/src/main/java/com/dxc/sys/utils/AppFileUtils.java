    ...
    public static ResponseEntity<Object> downloadFile(HttpServletResponse response,String path,String oldName){
        ...
        if(file.exists()){
            try{
                ...
                HttpHeaders haeder=new HttpHeaders();
                //封装响应内容类型(APPLICATION_OCTET_STREAM 响应的内容不限定)
                haeder.setContentType(MediaType.APPLICATION_OCTET_STREAM);
                //设置下载的文件名称
                haeder.setContentDispositionFormData("attachment",oldName);
                //创建ResponseEntity对象
                ResponseEntity<Object> entity=new ResponseEntity<Object>(bytes,haeder, HttpStatus.CREATED);
                return entity;
            }catch (Exception e){
                e.printStackTrace();
            }
